...
**Segment-level detail is not stored, and can no longer be recovered.** The per-night summary captures the signals that have consumers (stage durations + Efficiency + First/Last clock times for schedule and SRI). Raw segments would be 7-11k rows/year. Earlier revisions of this document said raw segments could be re-extracted from the archived Apple XML at `<root>/.processed/Export*.zip` — **that is no longer true.** `.processed/` is retired, the native export is not being produced any more, and the last cold `Export.zip` was deleted on 2026-08-16. There is nothing left to re-extract from: hypnograms and sleep-latency derivation are off the table for every window, historical and future alike.
