...
- `weekly_volume_per_muscle`: `{window_days: 28, current: {muscle: sets}, landmarks: {muscle: {mv, mev, mav, mrv}}}`. Fractional hard-set count via primary/synergist rules. **`current[muscle]` is the set sum over `window_days` (default 28); the landmarks are per-week.** Convert to per-week before comparing: `per_week_sets = current[muscle] / (window_days / 7)` (so divide by 4 for the default 28-day window). Name the band (MEV/MAV/MRV) on the **per-week** number, not the 28-day sum, otherwise volume reads ~4× higher than reality.
...
- **Volume analysis** → `weekly_volume_per_muscle.current[muscle] / (window_days / 7)` (per-week) vs `landmarks[muscle]`. Name the band (MEV/MAV/MRV) explicitly. Comparing the raw 28-day sum to weekly landmarks overstates volume ~4×.
