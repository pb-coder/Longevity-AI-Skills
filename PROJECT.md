...
**HealthAutoExport sleep limits.** HAE daily exports can include `Sleep Analysis [In Bed]`, and the importer writes it when present. The latest processed HAE ZIPs do not expose segment counts or first/last segment timestamps, so fragmentation and schedule-consistency fields remain structurally unavailable for HAE trackers rather than user-fixable blanks.
...
**Blank-start same-day thermal sessions.** If two complete same-shape entries have the same date, blank start, heat type, and cold type, the importer preserves both by assigning the later row a synthetic `Start` value like `occurrence:2`. Exact reruns stay idempotent; the synthetic occurrence is only used to avoid overwriting a complete existing session.
...
- **Apple Health import.** `/log` offers to refresh on every run via an `AskUserQuestion` prompt — pick "Refresh now" or "Skip"; the prompt always fires. The importer auto-resolves the export file from the workout-tracker root in this priority order: (1) `./Export - <Person>.zip` (Apple zipped XML, per-person), (2) `./Export.zip` (single-user fallback), (3) `./HealthAutoExport*.zip` (HealthAutoExport ZIP — most recent by mtime). Dispatch is by filename: `HealthAutoExport*.zip` → `import_health_auto_export.py --person <Person>`, `Export*.zip` → `import_apple_health.py --person <Person>`. **Both archive the source export to `<root>/.processed/` on success** (CSVs are the persistent record; the archive is the rollback path if a downstream bug damages the CSVs). Re-runs are idempotent; periodic store upserts preserve Notes, while machine-derived non-Notes cells are importer-owned. HealthAutoExport's `--replace-range` is available for authoritative backfills and requires `--allow-past-months` when spanning past monthly CSVs. If the resolved file's source disagrees with `profile.csv`'s `source`, the logger asks once before importing.
