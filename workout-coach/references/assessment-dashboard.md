...
```json
{
  ...
  "cards": {
    ...
    "trajectory_energy":               "one or two sentences. Quote the measured TDEE with its active/basal split, and the implied intake target as a number when a phase is open. Required when tracker JSON contains energy_28d. Intake is never observed here, only prescribed.",
...
All keys under `cards` are optional. A missing key emits a soft warning to stderr but doesn't block the render — the corresponding card simply renders without a callout. The full list of documented keys lives in `lib/render_validators.py::COACH_CARD_KEYS`, grouped by tab (Today, Trajectory domain callouts, Trajectory gated cards, and retained cross-tab cards). The validator suppresses the missing-callout warning for every key in `GATED_COACH_CARD_KEYS`, which holds four. Three are data-gated (`swim_trajectory_callout`, `trajectory_energy`, `nutrition_phase_callout`): their cards render only when the matching tracker block is present, respectively swim data, an `energy_28d` block, and an open nutrition phase, so a missing callout is noise when there is no card to attach it to. **The coach should still author those three whenever the data is present.** The fourth, `recovery_practices`, is in the set for a different reason: it renders an empty-state card and intentionally takes no callout.
...
| NEAT | `daily_activity_28d.{steps_daily_avg, exercise_min_daily_avg, walking_minutes_28d, walking_distance_km_28d, assessment}` | Own full card titled "NEAT over 28 days". Centered stat cells, all framed per day. **`steps_daily_avg` is the primary basis and leads the card**, carrying the colored status word (`high`→good, `moderate`→amber, `low`→warn) keyed off `assessment`, because steps accrue across the whole day without a workout being started and are the honest non-exercise channel. `exercise_min_daily_avg` is demoted to a secondary cell with no status word: Apple exercise minutes credit deliberate training, which is the thing NEAT is supposed to exclude. Walking min/day (`walking_minutes_28d / 28`) and walking km/day (`walking_distance_km_28d / 28`) stay as context cells; walking-workout counts are context only and never drive the band. When `steps_daily_avg` is null the status word falls back to the exercise-minutes cell. No coach callout. |
...
| Sleep | `sleep_summary.{means_h, sleep_efficiency_pct, absolute_sleep_note, fragmentation, schedule_consistency, resp_rate, breath_disturbances, outliers}` | Dedicated card after Health vitals. Hero shows average total + Time in Bed proxy. Stack chart breaks down Core / Deep / REM / Awake. Diagnostic rows (Deep+REM, continuity/efficiency, schedule, respiratory rate, breathing disturbances) each with a colored row-start dot + the value itself colored by band. Newer iOS/export paths may not emit explicit `InBed`, so derived values render as continuity rather than clinical efficiency. **The fragmentation row is retired.** It was derived from `n_segments`, which the current source no longer writes on any night, so `sleep_summary.fragmentation` is null on every night permanently. Render nothing for it rather than an empty row or a "no data" placeholder, and the coach must never describe fragmentation as improved: the number left because the field was retired, not because the nights got smoother. |
...
| Swim trajectory | `swim_summary.window_14d.*` (verdict, 14d aggregates, deltas vs prior 14d, PR flags), `swim_summary.css`, `swim_summary.css_retest_due`, `swim_summary.css_missing_nudge` | **Gated card — renders iff `swim_summary` is non-null in tracker JSON.** Trajectory tab; sits between Cardio domain and Recovery domain so the cardio narrative reads as one block. Hero = 14d session count + distance + improvement verdict ("getting better" / "regressing" / "mixed signals" / "holding steady" / "not enough swims yet"). Three trend rows: pace per 100m, SPL, SWOLF, each with current value + signed delta vs prior 14d (lower = better for all three; the renderer applies the sign-flip). Optional PR chips when current-14d best beats prior-14d best on pace or SWOLF. CSS context block when set; missing/retest prompt when CSS is absent or stale. Coach callout key: `swim_trajectory_callout`. Skipped silently for trackers without swim data. **The SWOLF row and the SWOLF PR chip are historical-only.** SWOLF and stroke style came off the per-lap payload of the retired XML export and the current source does not carry them, so those fields are populated only for swims imported before the retirement. Render the SWOLF row when the field is present and omit it entirely when it is not; do not draw an empty row. Per-workout swim aggregates (pool length, laps, strokes, SPL, distance, water temp) ARE still written, so pace per 100m, SPL and distance carry the card on current data. |
| Energy | `energy_28d.{tdee_kcal_daily_avg, active_kcal_daily_avg, basal_kcal_daily_avg, n_days, tdee_trend_kcal_per_week, basal_trend_kcal_per_week}` (the block also carries `n_active_days` / `n_basal_days` / `window_days` and the `tdee_trend` / `basal_trend` state blocks, which the card does not read), plus `nutrition_phase.energy.{tdee_kcal, target_deficit_kcal, implied_intake_kcal, basis}` when a phase is open | **Gated card — renders iff `energy_28d` is non-null in tracker JSON.** Trajectory tab; sits **directly above Nutrition phase**, because measured expenditure is the anchor the phase's intake target is derived from and the two read as one argument top to bottom. Hero = TDEE as kcal/day over `n_days`, with the active / basal split beneath it as two sub-values, so it is visible that basal is the larger share and active is the part training moves. Rows: (1) **TDEE trend**, `tdee_trend_kcal_per_week`, signed, muted when flat; (2) **Basal trend**, `basal_trend_kcal_per_week`, signed, and **amber when it is falling while a cut phase is open** — that is adaptive thermogenesis, the expenditure floor sinking toward the intake target, and it is the one number on this card that changes what the target should be; good or muted otherwise, and never warn, because adaptation is expected rather than alarming. The scalar is null unless the fit resolved, so the row is omitted rather than drawn flat when the direction is unresolved, and there is deliberately **no noise floor** on the amber: any resolved negative slope during an open cut flags, with the magnitude stated in the sublabel so the reader sizes it themselves; (3) **Implied intake target**, rendered only when `nutrition_phase.energy` exists, as `implied_intake_kcal` labelled with its `basis` (`measured_28d` reads as measured; anything else reads as estimated). Row 3 renders with a muted "target, not tracked" qualifier every time it renders: **there is no intake data anywhere in this tracker**, so the number is a prescription and the card must never let it read as an observation. Coach callout key: `trajectory_energy`. Skipped silently for trackers with no energy rows. |
...
```json
{
  "headline": "Recovery looks firm and HRV is well above baseline. Sleep slipped under 7 hours this week and bodyweight is drifting down faster than a hypertrophy goal can absorb. Hold loads, target 3400 kcal a day against a measured 3204 TDEE, and get one earlier night.",
  "cards": {
    ...
    "trajectory_energy":  "Measured TDEE 3204 kcal a day, 1058 of it active. Basal is holding flat, so the number to eat against is stable. Nothing here logs intake, so treat any target as a prescription and read the scale for whether it lands.",
    "vitals":             "Two signals to act on: sleep under 7 hours and weight dropping 0.56 kg per week. Earlier night tonight, and if the drop is unintentional eat to 3400 rather than the measured 3204 TDEE.",
