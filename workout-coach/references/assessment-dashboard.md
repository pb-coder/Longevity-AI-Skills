...
| Health vitals | `health_metrics_weekly` (HRV / RHR / wrist temp / sleep / deep / REM / VO2max series), `vo2max_latest`, `vo2max_trend_per_4w`, `bodyweight_latest`, `bodyweight_trend_kg_per_week`, `bodyweight_weekly` | One clean table, no inline coach rows. Sparklines colored by per-row status. **Wrist temp** State is computed dynamically (stable / rising / elevated / insufficient data) from the latest week's z-score vs. the prior 3-week mean, matching the HRV/RHR pattern but inverted (higher = warning). **Bodyweight** carries a sparkline driven by the new `bodyweight_weekly` series (ISO-week means). When an open nutrition phase exists, `bodyweight_trend_kg_per_week` is scoped to the phase start date; use `nutrition_phase.actuals.rate_kg_per_wk_14d` as the primary phase-status number. Sparkline column hides at ≤ 480 px. Column widths set via `.vitals-table` class. |
...
| Recovery practices | `thermal_summary.heat`, `thermal_summary.cold`, `thermal_summary.cold.recent_sessions`, `thermal_summary.adherence`, `light_therapy_summary` | Three sub-cards, identical layout. Cold sub-card lists recent sessions with their temperature (the `dose_hint: "amber"` flag for cold_air ≥ 18 °C tags weak doses). Heat and light adherence targets come from `profile.csv` when configured, with defaults only as fallback; coach copy should say "below configured target" rather than implying universal reachability. |
...
Bodyweight signal hierarchy when a nutrition phase is open: `nutrition_phase.actuals.rate_kg_per_wk_14d` drives phase status, `bodyweight_trend_kg_per_week` is phase-scoped support, `bodyweight_latest` is the newest reading, and `bodyweight_weekly` / week-over-week means are visual context only.
