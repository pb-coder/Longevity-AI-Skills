...
**Repeated split days in one week.** When a split day recurs in the week (the two Upper or two Lower days, or successive Full Body days), the repeats must not be clones. Keep one actively progressing anchor per muscle, but vary the second exposure: change the lead angle/pattern and swap the isolation and arm slots (e.g. first Upper leads flat press + pushdown, second leads incline press + overhead triceps; pair vertical-pull emphasis on one day with horizontal-row emphasis on the other).
...
- **Reserve direct arm work: ≥6 working sets/week for biceps AND ≥6 for triceps**, spread
  across the week (not one session), never a workout's final bullet. Pressing/pulling synergy
  does NOT count toward this floor — the volume model credits +0.5 biceps per pull and +0.5
  triceps per press, which can clear MEV on paper while direct arm growth gets nothing (§8;
  triceps need dedicated volume beyond pressing synergy, Baz-Valle 2022). **Rotate the arm
  exercise** across sessions and blocks — do not repeat the same single curl/extension every
  plan; when introducing a fresh variant, start conservative (leave 2-3 in reserve). The
  render validator counts direct arm sets and warns on undershoot; treat that warning as
  binding outside an explicit deload.
...
- **Split selection**: frequency-driven per the split-selection rule at the top of this section (§14). Never keep a split that trains muscles <2x/week.
...
Before finalizing a plan or dashboard, skim **`references/common-mistakes.md`** — the full catalog of failure modes (planning/programming, data-reading, dashboard-gating) with the correct behavior for each. The load-bearing planning rules in it (core dose, direct arm dose, frequency-driven split, deload cadence, stall response) are already enforced inline above and by the render validators; the reference is the complete backstop.
