...
- ``validate_workout_md(text, session_recommendation=None) -> (errors,
  warnings)`` — validates the lean workout markdown's COPY (em-dashes,
  off-catalog names) before it is embedded in the dashboard. When given
  the gate's ``session_recommendation``, also enforces that a no-strength
  gate (substitute kind ``rest`` or ``zone_2``) is not undercut by
  strength working sets in the markdown.
