# Exercises Database

Organized by primary muscle group > movement pattern. Tags:
- `[BB]` Barbell `[DB]` Dumbbell `[Cable]` `[Machine]` `[BW]` Bodyweight `[Smith]` `[LM]` Landmine `[Band]`
- `+muscle` = synergist (coach counts as 0.5 set for that muscle)
- `◆` = lengthened-position emphasis (prioritize per §5 of training science)

---

## WARMUP

### Full Body
- Jumping Jacks [BW]
- Jump Rope [Rope]
- High Knees [BW]
- Butt Kicks [BW]
- World's Greatest Stretch [BW]
- Burpee [BW]
- Cat-Cow Stretch [BW]
- Hip 90/90 Stretch [BW]

### Upper Body
- Arm Circles [BW]
- Band Pull-Apart [Band]
- Band Dislocates [Band]
- Dead Hang [BW]
- Scapular Pull-Up [BW]
- Pull-Up (warmup) [BW]

### Lower Body
- Bodyweight Squat [BW]
- Glute Bridge [BW]

---

## CHEST

### Horizontal Push (Compound)
- Flat Barbell Bench Press [BB] — +front delt, +triceps
- Incline Barbell Bench Press [BB] — +front delt, +triceps
- Dumbbell Flat Bench Press [DB] — +front delt, +triceps
- Dumbbell Incline Bench Press [DB] — +front delt, +triceps
- Dumbbell Floor Press [DB] — +front delt, +triceps
- Chest Press Machine [Machine] — +front delt, +triceps
- Cable Chest Press [Cable] — +front delt, +triceps
- Single Arm Cable Press [Cable] — +front delt, +triceps
- Cable Incline Press [Cable] — +front delt, +triceps
- Smith Machine Bench Press [Smith] — +front delt, +triceps
- Smith Machine Incline Bench Press [Smith] — +front delt, +triceps
- Incline Chest Press Machine [Machine] — +front delt, +triceps
- Decline Chest Press Machine [Machine] — +triceps
- Push-Up [BW] — +front delt, +triceps
- Dip [BW] — +front delt, +triceps ◆

### Chest Isolation
- Pec Deck [Machine]
- Inner Chest Press Machine [Machine]
- Dumbbell Fly [DB] ◆
- Cable Fly (Low to High) [Cable]
- Cable Fly (High to Low) [Cable]
- Cable Crossover [Cable]
- Cable Low to High Fly [Cable]
- Cable Decline Fly [Cable]
- Dumbbell Pullover [DB] — +lats

---

## BACK

### Vertical Pull (Compound)
- Pull-Up [BW] — +biceps ◆
- Chin-Up [BW] — +biceps ◆
- Cable Lat Pulldown [Cable] — +biceps ◆
- Cable Close-Grip Pulldown [Cable] — +biceps
- Cable Wide Grip Lat Pulldown [Cable] — +biceps
- Cable V-Bar Pulldown [Cable] — +biceps
- Cable Reverse Grip Pulldown [Cable] — +biceps
- Lat Pulldown Machine [Machine] — +biceps
- Pullover Machine [Machine] — +chest ◆
- Straight-Arm Pulldown [Cable]

### Horizontal Pull (Compound)
- Barbell Row [BB] — +biceps, +rear delt
- Pendlay Row [BB] — +biceps, +rear delt
- Dumbbell Row [DB] — +biceps, +rear delt
- Cable Seated Row [Cable] — +biceps, +rear delt
- Seated Row Machine [Machine] — +biceps, +rear delt
- Chest Supported Row Machine [Machine] — +biceps, +rear delt
- Chest Supported T-Bar Row [BB] — +biceps, +rear delt
- Cable Single Arm Row [Cable] — +biceps, +rear delt
- Straight Arm Cable Row [Cable]
- Smith Machine Row [Smith] — +biceps, +rear delt
- Seal Row [BW] — +biceps, +rear delt
- Landmine Row [LM] — +biceps, +rear delt

### Back — Other
- Conventional Deadlift [BB] — +glutes, +hamstrings, +erectors (primary: posterior chain)
- Sumo Deadlift [BB] — +glutes, +quads, +erectors
- Trap Bar Deadlift [BB] — +glutes, +quads
- Deadlift Machine [Machine] — +glutes, +hamstrings, +quads
- 45 Degree Back Extension [Machine] — +glutes, +hamstrings (primary: erectors/lower back)
- Back Extension Machine [Machine] — +glutes, +hamstrings (primary: erectors/lower back)
- Superman [BW]

---

## SHOULDERS

### Vertical Push (Compound)
- Barbell Overhead Press [BB] — +triceps, +upper chest
- Dumbbell Shoulder Press [DB] — +triceps
- Shoulder Press Machine [Machine] — +triceps
- Smith Machine Shoulder Press [Smith] — +triceps
- Barbell Push Press [BB] — +triceps
- Landmine Press [LM] — +triceps, +upper chest
- Viking Press Machine [Machine] — +triceps, +upper chest

### Lateral Delt (Isolation)
- Dumbbell Lateral Raise [DB]
- Cable Lateral Raise [Cable] ◆
- Lateral Raise Machine [Machine]
- Cable Front Raise [Cable]
- Dumbbell Front Raise [DB]
- Cable Upright Row [Cable] — +traps

### Rear Delt (Isolation)
- Rear Delt Fly Machine [Machine]
- Dumbbell Rear Delt Fly [DB]
- Cable Face Pull [Cable] — +external rotators
- Cable Rear Delt Row [Cable]

### Traps
- Dumbbell Shrug [DB]
- Cable Shrug [Cable]

---

## BICEPS

### Compound Contribution
(Biceps receive ~0.5 sets from every pull compound. See BACK section.)

### Isolation — Lengthened Position
- Incline Dumbbell Curl [DB] ◆
- Bayesian Cable Curl [Cable] ◆
- Overhead Bicep Curl Machine [Machine] ◆

### Isolation — Standard
- Dumbbell Bicep Curl [DB]
- Barbell Curl [BB]
- EZ Bar Curl [BB]
- Dumbbell Hammer Curl [DB]
- Concentration Curl [DB]
- Seated Dumbbell Curl [DB]
- Zottman Curl [DB]
- Cross Body Hammer Curl [DB]
- Alternate Dumbbell Curl [DB]
- Cable Bicep Curl [Cable]
- Cable Rope Hammer Curl [Cable]
- High Cable Curl [Cable]
- Single Arm Cable Curl [Cable]
- Cable Rope Curl [Cable]
- Cable Concentration Curl [Cable]
- Seated Cable Curl [Cable]
- Close Grip Barbell Curl [BB]
- Wide Grip Barbell Curl [BB]
- Drag Curl [BB]

### Isolation — Shortened Position
- Spider Curl [DB]
- Preacher Curl (Dumbbell) [DB]
- Preacher Curl Machine [Machine]
- Cable Preacher Curl [Cable]

### Forearms
- Reverse Barbell Curl [BB]
- Dumbbell Reverse Curl [DB]
- Cable Reverse Curl [Cable]

---

## TRICEPS

### Compound Contribution
(Triceps receive ~0.5 sets from every press compound. See CHEST and SHOULDERS sections.)

### Isolation — Lengthened Position
- Cable Overhead Tricep Extension [Cable] ◆
- Dumbbell Tricep Overhead Extension [DB] ◆
- Cable Overhead Extension [Cable] ◆
- Barbell Skull Crusher [BB] ◆

### Isolation — Standard
- Cable Tricep Pushdown [Cable]
- Dumbbell Kickback [DB]
- Cable Kickback [Cable]
- Tricep Extension Machine [Machine]
- Tricep Dip Machine [Machine] — +chest, +front delt

---

## QUADS

### Squat Pattern (Compound)
- Barbell Back Squat [BB] — +glutes, +erectors ◆ (deep)
- Barbell Front Squat [BB] — +core ◆ (deep)
- Dumbbell Goblet Squat [DB] — +core ◆
- Dumbbell Bulgarian Split Squat [DB] — +glutes ◆
- Hack Squat [Machine] — +glutes ◆ (deep)
- Belt Squat Machine [Machine] — +glutes ◆
- Vertical Leg Press [Machine] — +glutes ◆
- Pendulum Squat [Machine] — +glutes ◆
- Smith Machine Squat [Smith] — +glutes
- Cable Squat [Cable]
- Cable Bulgarian Split Squat [Cable] — +glutes
- Cable Goblet Squat [Cable]
- Pistol Squat [BW] — +glutes ◆
- Sissy Squat [BW] ◆
- Sissy Squat Machine [Machine] ◆
- Dumbbell Lunge [DB] — +glutes
- Cable Front Lunge [Cable] — +glutes
- Cable Reverse Lunge [Cable] — +glutes
- Cable Step-Up [Cable] — +glutes
- Bulgarian Split Squat Jump [BW] — +glutes

### Quad Isolation
- Leg Extension [Machine]
- Leg Press [Machine] — +glutes (foot placement dependent)

---

## HAMSTRINGS

### Hip Hinge (Compound)
- Romanian Deadlift [BB] — +glutes, +erectors ◆
- Dumbbell Romanian Deadlift [DB] — +glutes ◆
- Single Leg Romanian Deadlift [DB] — +glutes ◆
- Cable Romanian Deadlift [Cable] — +glutes ◆
- Smith Machine Romanian Deadlift [Smith] — +glutes ◆
- Barbell Good Morning [BB] — +erectors ◆

### Hamstring Isolation
- Leg Curl (Lying) [Machine]
- Leg Curl (Seated) [Machine] ◆
- Standing Leg Curl Machine [Machine]
- Cable Standing Leg Curl [Cable]
- Nordic Hamstring Curl [BW] ◆

---

## GLUTES

### Compound
(Glutes receive major contribution from squats and hip hinges. See QUADS and HAMSTRINGS.)
- Barbell Hip Thrust [BB]
- Single Leg Hip Thrust [BB]
- Hip Thrust Machine [Machine]
- Cable Hip Thrust [Cable]
- Banded Hip Thrust [Band]
- Reverse Hyperextension Machine [Machine] — +hamstrings, +erectors

### Isolation
- Cable Kickback (Glutes) [Cable]
- Glute Kickback Machine (Standing) [Machine]
- Glute Kickback Machine (Kneeling) [Machine] ◆
- Hip Extension Machine [Machine]
- Hip Abductor Machine [Machine]
- Cable Hip Abduction [Cable]

---

## ADDUCTORS
- Hip Adductor Machine [Machine]
- Cable Hip Adduction [Cable]

---

## CALVES
- Calf Raise Machine [Machine]
- Seated Calf Raise [Machine]
- Donkey Calf Raise Machine [Machine] ◆
- Cable Standing Calf Raise [Cable]

---

## CORE

### Anti-Extension
- Plank [BW]
- Dead Bug [BW]
- Hollow Body Hold [BW]
- Ab Crunch Machine [Machine]

### Anti-Rotation
- Cable Pallof Press [Cable]
- Bird Dog [BW]

### Flexion
- Leg Raise [BW]
- Captain's Chair Knee Raise [Machine]
- Kneeling Cable Crunch [Cable]
- Standing Cable Crunch [Cable]
- Cable Reverse Crunch [Cable]
- Bicycle Crunch [BW]
- V-Up [BW]
- Flutter Kick [BW]
- Mountain Climber [BW]

### Rotation
- Cable Woodchop (High to Low) [Cable]
- Cable Woodchop (Low to High) [Cable]
- Cable Russian Twist [Cable]
- Russian Twist [BW]
- Rotary Torso Machine [Machine]
- Landmine Rotation [LM]
- Roman Chair Side Bend [Machine]

### Isometric
- L-Sit [BW]
- Side Plank [BW]

---

## NECK
- Neck Flexion Machine [Machine]
- Neck Extension Machine [Machine]
- Neck Bridge [BW]

---

## FULL BODY (Compound)
- Barbell Thruster [BB]
- Dumbbell Thruster [DB]
- Barbell Clean [BB]
- Barbell Snatch [BB]
- Dumbbell Farmer Walk [DB]
- Box Jump [BW]
- Broad Jump [BW]
- Tuck Jump [BW]

---

## CARDIO
- Treadmill Run [Treadmill]
- Rowing Machine [Rower]
- Elliptical [Elliptical]
- Stationary Bike [Bike]
- Stair Climber [Stair Climber]
- Assault Bike [Assault Bike]
- Hike [Outdoor]
- Swim [Pool]
- Walk [Outdoor]
- Outdoor Run [Outdoor]
- Outdoor Cycling [Outdoor]
- HIIT [Various]

---

## WELLNESS
- Yoga [Mat]
- Stretching [Mat]
