...
- `dose_staleness`: per carried-forward exercise, whether load or reps actually moved between generations, with materiality floors so a rounding change does not read as progress. Measured baseline was 70% of carried exercises returning with an unchanged dose; the target is under 40%.
...
- `core_week_spec`: `sets_per_session` (4 lower / 2 upper), `min_distinct_exercises_per_week` 3, `min_pattern_categories_per_week` 3, `max_sessions_per_exercise_per_week` 2, `min_loaded_flexion_exercises_per_week` 1. Pattern categories are the CORE subsections in `exercises-database.md` (Flexion / Anti-Extension / Anti-Rotation / Anti-Lateral-Flexion / Rotation).
...
**Bodyweight line.** One line at the bottom of this section. **Read `bodyweight_trend.state` first.**

- `state: "unresolved"` — say so and stop. `Bodyweight: 76.1kg (2026-04-21). No resolved direction yet: {note}.` Do not report a gain or a loss, do not quote `point_kg_per_week` as if it were the rate, and do not say "trending down slightly". The reason is in `reason` / `note`; paraphrase it in plain English.
...
- **Tier scaling is applied for you, per workout index.** `downgrade` scales the budget to **0.6 of the base for the whole session**, and only for workouts before `expected_rebound_by_session`; later workouts keep the full budget. `reactive_deload` scales the whole week to 0.5, Tier A to 0. Read the scaled number rather than doing your own arithmetic, and note it is a whole-session scale, not "25% off the secondaries": the isolation block absorbs the cut because core and arms cannot.
...
- `workout_set_budget_warnings` is the **only non-binding check** in the plan validator: an intentional deload legitimately undershoots the budget, so it surfaces as a warning and the render proceeds. Outside a deload or downgrade tier, treat it as a defect and fix it before writing. The core and arm findings are the blocking ones.
...
- **Recovery-aware adjustments (§18):** **Lead with `recovery.score`** — it already folds HRV, RHR, sleep total, sleep stages, sleep consistency, wrist temp, and HR Recovery into one renormalized weighted average of personal z-scores (5 = personal average; VO2max trend is **not** in the score — it lives separately in `vo2max_latest` / `vo2max_trend_per_4w` for the cardio check). Apply:
  ...
  - **Hold loads does not mean hold the dose.** `dose_staleness` wants under 40% of carried exercises returning an identical prescription, and with load frozen the intended lever is the **rep target**: carry the weight forward and advance the rep range or the rep goal within it, which is exactly what double progression asks for under a hold. That is a real dose change and it counts as one. Churning set counts to move the metric does not: adding or dropping a set purely to make the prescription differ changes weekly volume for no training reason and breaks the tier targets. If a lift genuinely has nowhere to go on either axis, leave it and let it read as stale rather than manufacturing a change.
  - **Cold-start escape hatch.** Copy-forward has nothing to copy for a movement with no logged history, and taken literally it makes every new movement illegal to prescribe. It does not apply to them. For a movement absent from the log, its derived starting load in `rotation_candidates.candidates[]` **overrides the copy-forward rule** and is the load you write, at that block's `target_reps`. It already carries the first-exposure discount, so do not discount it again; write it as prescribed and let the next session's log take over. **When `load_kg` is null, `load_basis` decides what to do and there are three cases, not one.** `bodyweight` (which also carries `unit: "bodyweight"`) means the movement has no external load: prescribe reps or seconds, not a weight. `no_reference` (no sibling with logged history) and `unknown_transfer` (a sibling exists but no coefficient covers the equipment pair) both mean the pipeline could not derive a safe number — **they are not a bodyweight signal**, and prescribing them unloaded strips the load off a loaded lift. In those two cases pick a conservative weight yourself, one controllable for `target_reps` with 2-3 reps in reserve, and say so in one sub-bullet cue. A movement with no candidate entry at all is the same instruction: treat it as a genuine first exposure, prescribe conservatively, and cue it.
...
**Core training (§24).** The countable part is `core_week_spec`, enforced as blocking render errors: per-session dose, distinct exercises, pattern categories, per-exercise frequency, and one loaded flexion movement a week. Do not restate any of it here or reason about it as if it were advisory. Select from the rotation-pool structure in §24.3: one loaded flexion slot, one rotating non-flexion slot cycled by category across blocks.

What the spec cannot decide, and you must:
- **Placement.** Core goes inside the isolation/accessory block, supersetted with an unrelated isolation movement (curls, lateral raises, calves, triceps). Never the final bullet, never before a compound. Order does not affect hypertrophy (§24.2), but the last slot is where prescriptions go unperformed, and moving core ahead of the compounds costs the compounds for no gain.
...
**Count the sets in that example: 22 working sets** (4 bench + 4 shoulder press + 4 fly + 2 core + 4 lateral raise + 4 triceps; the two ramp sets and the two prep movements don't count). That is not decoration — it lands exactly on a 22-set budget, and the example is written to be copied. An example that models 18 sets teaches an 18-set session. This is an UPPER day, so its core allocation is 2 sets; a LOWER day carries 4. Note where they sit: inside the cable block, supersetted with an unrelated isolation movement, never the last line. One session cannot show the weekly core distribution — read `core_week_spec` for that.

### Cardio sessions (only when prescribed)

Written as their own sections after the strength workouts, not mixed in. Two shapes, both `### Cardio N: <kind> (<duration>)` followed by plain bullets:
