...
class SynergistOffsetPinTests(unittest.TestCase):
    ...
    def test_external_rotators_is_measured_not_ambiguous(self) -> None:
        ...
        #
        # What is actually true is a SATISFIABILITY problem, and the
        # comment now says so: the catalog has no primary entry for the
        # muscle, so the restated MEV cannot be hit until it gains one.
        ...
        self.assertNotIn("external_rotators", primaries,
                         "the catalog gained a primary external-rotator "
                         "entry; the target is hittable now and the "
                         "UNHITTABLE note in constants.py is stale")
    ...
    def test_the_ambiguous_marker_means_exactly_one_thing(self) -> None:
        ...
        ambiguous = src.split("# AMBIGUOUS —")[1].split("# UNHITTABLE")[0]
